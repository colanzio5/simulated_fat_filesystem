...
BlockDevice::result BlockDevice::readBlock(uint32_t block, void *buffer)
{
	...
	if (retval == BlockDevice::success)
	{
		...
		if (retval == BlockDevice::badreadwrite)
		{
			...
			if (seekend == BlockDevice::success)
			{
				...
				char nullchar = 0;
				retval = (fwrite(&nullchar, sizeof(char), 1, this->file_h) == 1) ? BlockDevice::success : BlockDevice::badreadwrite;
...
BlockDevice::result BlockDevice::writeBlock(uint32_t block, const void *buffer)
{
